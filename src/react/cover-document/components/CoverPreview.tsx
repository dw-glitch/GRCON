...
export function CoverPreview({
  ...
  return (
    <section className="cover-card cover-preview-card" aria-labelledby="cover-preview-heading">
      <div className="cover-card-heading"><div><span>3 — PREVIEW E SAÍDA</span><h3 id="cover-preview-heading">Capa + contracapa/página seguinte real</h3></div><small>{totalPages ? `Folha 1 de ${totalPages}` : "Aguardando total de folhas"}</small></div>
      <div className="cover-preview-frame">
        {previewUrl ? <iframe src={previewUrl} title="Prévia real da capa e da contracapa/página seguinte" /> : <div className="cover-preview-placeholder"><strong>Prévia indisponível</strong><span>Selecione a LD, o documento e resolva os campos obrigatórios.</span></div>}
      </div>
      ...
      <small className="cover-output-note">No modo Substituir a capa existente, a segunda página exibida nesta prévia é a contracapa real do PDF anexado. Ela é preservada sem alteração; o GRCON não aplica contracapa universal.</small>
    </section>
  );
}
