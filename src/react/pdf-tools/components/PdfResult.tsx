...
export function PdfResult({ result, formattedSize, onDownload }: PdfResultProps) {
  return (
    <section className="pdf-merge-result" hidden={!result} id="pdf-merge-result" aria-live="polite">
      ...
      <div className="pdf-merge-result-copy">
        <span>PDF combinado com sucesso</span>
        <strong id="pdf-merge-result-name" title={result?.name}>{result?.name || "PDF_Combinado.pdf"}</strong>
        <small id="pdf-merge-result-meta">
          {result
            ? `${result.pageCount.toLocaleString("pt-BR")} página(s) · ${formattedSize} · ${result.fileCount.toLocaleString("pt-BR")} PDF(s) utilizados`
            : "Arquivo pronto"}
        </small>
      </div>
