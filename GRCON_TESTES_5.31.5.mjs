...
check("migrações contêm trava, exclusão lógica e reserva idempotente", () => {
  const previous = fs.readFileSync(path.join(root, "SUPABASE_MIGRACAO_5.31.4.sql"), "utf8");
  const sql = fs.readFileSync(path.join(root, "SUPABASE_MIGRACAO_5.31.5.sql"), "utf8");
  assert.match(previous, /grcon_history_workspace_egrdt_number_uidx/i);
  assert.match(sql, /pg_advisory_xact_lock/i);
  assert.match(sql, /deleted_at timestamptz/i);
  assert.match(sql, /target_request_id uuid/i);
  assert.match(sql, /grcon_egrdt_reservations_request_item_uidx/i);
  assert.match(sql, /grcon_egrdt_reservations_reserved_by_idx/i);
  assert.match(sql, /history_id uuid/i);
  assert.match(sql, /status = 'consumed'/i);
  assert.match(sql, /revoke all on function public\.grcon_fill_profile_fields\(\) from public, anon, authenticated/i);
  assert.match(sql, /revoke all on function public\.grcon_sync_auth_user_profile\(\) from public, anon, authenticated/i);
});
...
check("service worker publica o cache isolado da versão 5.31.5", () => {
  const source = fs.readFileSync(path.join(root, "sw.js"), "utf8");
  assert.match(source, /grcon-v5\.31\.5/);
...
console.log(JSON.stringify({ version: "5.31.5", passed: true, checks: checks.length, names: checks }, null, 2));
