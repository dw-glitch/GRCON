(function (root) {
  ...
  const state = {
    ...
    comparison: null,
    listMode: "sigem-new",
    filters: { query: "", documentClass: "", revision: "", status: "", discipline: "", tag: "", eap: "", source: "" },
    page: 1,
    filteredRows: [],
    bound: false,
    eventTimer: null,
  ...
  function selected(system, role) {
    ...
    return list.find((item) => item.id === state.selections[key]) || null;
  }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      #${SECTION_ID}.spw-evo-v2{margin-top:16px;min-width:0}.spw-evo-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:10px}.spw-evo-head h3{margin:0;color:var(--text-strong,#183247);font-size:1.08rem}.spw-evo-head p{margin:4px 0 0;max-width:820px;color:var(--text-muted,#66798a);font-size:.75rem;line-height:1.45}.spw-evo-actions{display:flex;gap:7px;flex-wrap:wrap}.spw-evo-scope{display:flex;align-items:center;gap:7px;margin:0 0 10px;padding:8px 10px;border:1px solid var(--border,#dce4eb);border-radius:10px;background:var(--surface-soft,#f7fafc);font-size:.7rem;color:var(--text-muted,#66798a)}.spw-evo-scope strong{color:var(--text-strong,#294258)}
      .spw-evo-selectors{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:10px}.spw-evo-source{padding:10px 11px;border:1px solid var(--border,#dce4eb);border-radius:11px;background:var(--surface,#fff)}.spw-evo-source header{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:8px}.spw-evo-source header strong{font-size:.79rem;color:var(--text-strong,#294258)}.spw-evo-source header small{font-size:.64rem;color:var(--text-muted,#66798a)}.spw-evo-pair{display:grid;grid-template-columns:1fr auto 1fr;gap:7px;align-items:end}.spw-evo-pair label{display:grid;gap:4px}.spw-evo-pair label span{font-size:.58rem;font-weight:900;text-transform:uppercase;color:var(--text-muted,#66798a)}.spw-evo-pair select{min-height:36px;min-width:0}.spw-evo-arrow{padding-bottom:9px;color:var(--text-muted,#66798a);font-weight:900}
      .spw-evo-kpis{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.spw-evo-kpi{appearance:none;text-align:left;border:1px solid var(--border,#dce4eb);border-radius:13px;background:var(--surface,#fff);padding:13px;cursor:pointer;box-shadow:0 5px 18px rgba(32,56,85,.04)}.spw-evo-kpi:hover,.spw-evo-kpi:focus-visible{border-color:var(--brand-300,#8ac4df)}.spw-evo-kpi span{display:block;font-size:.62rem;font-weight:900;text-transform:uppercase;color:var(--text-muted,#66798a)}.spw-evo-kpi strong{display:block;margin-top:5px;font-size:1.65rem;color:var(--text-strong,#17324a)}.spw-evo-kpi small{display:block;margin-top:4px;font-size:.68rem;color:var(--text-muted,#66798a)}.spw-evo-kpi.sigem{border-left:4px solid var(--spw-sigem,#0b7895)}.spw-evo-kpi.pw{border-left:4px solid var(--spw-pw,#6d4ac7)}.spw-evo-kpi.pending{border-left:4px solid var(--warning-500,#c68a28)}
      ...
      .spw-evo-filters{display:grid;grid-template-columns:2fr repeat(6,minmax(105px,1fr));gap:7px;padding:9px;border:1px solid var(--border,#dce4eb);border-radius:10px;background:var(--surface,#fff)}.spw-evo-filters label{display:grid;gap:4px}.spw-evo-filters span{font-size:.57rem;font-weight:900;text-transform:uppercase;color:var(--text-muted,#66798a)}.spw-evo-filters input,.spw-evo-filters select{min-height:34px;min-width:0}.spw-evo-list-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:8px}.spw-evo-list-head strong{font-size:.78rem;color:var(--text-strong,#294258)}.spw-evo-list-head small{font-size:.66rem;color:var(--text-muted,#66798a)}
      .spw-evo-table-wrap{overflow:auto;max-height:520px;margin-top:6px;border:1px solid var(--border,#dce4eb);border-radius:10px;background:var(--surface,#fff)}.spw-evo-table{width:100%;border-collapse:collapse;font-size:.68rem}.spw-evo-table th,.spw-evo-table td{padding:8px 9px;border-bottom:1px solid var(--border,#edf1f4);text-align:left;white-space:nowrap}.spw-evo-table thead th{position:sticky;top:0;z-index:1;background:var(--surface-soft,#f6f9fb);font-size:.56rem;text-transform:uppercase;color:var(--text-muted,#66798a)}.spw-evo-table tbody tr{cursor:pointer}.spw-evo-table tbody tr:hover{background:var(--surface-soft,#f8fafc)}.spw-evo-empty{padding:26px 14px;text-align:center;color:var(--text-muted,#66798a);font-size:.72rem}.spw-evo-empty strong{display:block;color:var(--text-strong,#294258);margin-bottom:3px}.spw-evo-pager{display:flex;justify-content:flex-end;gap:6px;align-items:center;margin-top:7px}.spw-evo-pager button{min-height:31px}.spw-evo-pager span{font-size:.67rem;color:var(--text-muted,#66798a)}
      ...
      @media(max-width:1050px){.spw-evo-filters{grid-template-columns:repeat(4,1fr)}.spw-evo-filters label:first-child{grid-column:1/-1}}@media(max-width:760px){.spw-evo-head{display:grid}.spw-evo-selectors,.spw-evo-audit,.spw-evo-kpis{grid-template-columns:1fr}.spw-evo-filters{grid-template-columns:1fr 1fr}.spw-evo-filters label:first-child{grid-column:1/-1}}@media(max-width:480px){.spw-evo-pair{grid-template-columns:1fr}.spw-evo-arrow{display:none}.spw-evo-filters,.spw-evo-detail{grid-template-columns:1fr}}
    `;
    document.head.appendChild(style);
  }

  function option(snapshot) {
    const label = `${fmtDate(snapshot.importedAt)} · ${snapshot.fileName || "base"} · ${fmt(snapshot.audit?.acceptedRecords || 0)} registros`;
    return `<option value="${esc(snapshot.id)}">${esc(label)}</option>`;
  }
  function pairDefaults(system) {
    const list = ordered(state[system]);
    if (!list.length) return;
    const currentKey = `${system}Current`, prevKey = `${system}Prev`;
    ...
    if (!list.some((row) => row.id === state.selections[prevKey])) state.selections[prevKey] = currentIndex > 0 ? list[currentIndex - 1].id : "";
  }
  ...
  async function readLdUniverse(force) {
    ...
    if (!files.length) {
      state.ldUniverse = Core().buildLdUniverse([], []);
  ...
  async function buildPreparedSnapshots(system, metadata, payloads, universe) {
    ...
    for (const sourceSnapshot of ordered(metadata)) {
      ...
      try {
        output.push(Core().buildSnapshot(system, base, universe, { snapshotId: sourceSnapshot.id, sourceSnapshotId: sourceSnapshot.id }));
  ...
  async function loadSnapshots(forceLd) {
    ...
    try {
      ...
      state.sigem = sigem.output; state.pw = pw.output;
  ...
  function recalculate() {
    const sp = selected("sigem", "previous"), sc = selected("sigem", "current"), pp = selected("pw", "previous"), pc = selected("pw", "current");
    state.comparison = Core().comparePeriod(sp, sc, pp, pc);
    state.page = 1;
    render();
  }
  ...
  function ensureSection() {
    ...
    section.innerHTML = `
      <header class="spw-evo-head"><div><span class="spw-kicker">EVOLUÇÃO</span><h3>Movimentação real entre bases</h3><p>Mostra novos registros, saídas e o que apareceu no SIGEM e ainda não foi identificado no ProjectWise. Nova revisão do mesmo documento conta como nova ocorrência.</p></div><div class="spw-evo-actions"><button class="secondary-button" id="spw-evo-export" type="button">Exportar lista</button><button class="text-button" id="spw-history-manage" type="button">Gerenciar histórico</button></div></header>
      <div class="spw-evo-scope" id="spw-evo-scope"></div>
      <div class="spw-evo-selectors" id="spw-evo-selectors"></div>
      <div class="spw-evo-kpis" id="spw-evo-kpis"></div>
      <div class="spw-evo-net" id="spw-evo-net"></div>
      <div class="spw-evo-audit" id="spw-evo-audit"></div>
      <nav class="spw-evo-tabs" id="spw-evo-tabs" aria-label="Listas da evolução"></nav>
      <div class="spw-evo-filters">
        ...
        <label><span>Tipo</span><select id="spw-evo-filter-class"><option value="">Todos</option><option>ET</option><option>N-1710</option><option>CV</option></select></label>
      ...
      <div class="spw-evo-list-head"><div><strong id="spw-evo-list-title"></strong><br><small id="spw-evo-list-subtitle"></small></div><small id="spw-evo-list-count"></small></div>
      <div class="spw-evo-table-wrap" id="spw-evo-table"></div><div class="spw-evo-pager" id="spw-evo-pager"></div>
    ...
    const navButton = document.querySelector('[data-spw-jump="spw-history-section"]');
    if (navButton) navButton.textContent = "Evolução";
    return section;
  }
  ...
  function renderSelectors() {
    ...
    const block = (system, label) => {
      const list = ordered(state[system]);
      const previousKey = `${system}Prev`, currentKey = `${system}Current`;
      return `<section class="spw-evo-source"><header><strong>${label}</strong><small>${fmt(list.length)} base(s) utilizável(is)${state.unavailable[system] ? ` · ${fmt(state.unavailable[system])} antiga(s) sem payload bruto` : ""}</small></header><div class="spw-evo-pair"><label><span>Base anterior</span><select data-evo-select="${previousKey}"><option value="">— sem anterior —</option>${list.map(option).join("")}</select></label><span class="spw-evo-arrow">→</span><label><span>Base atual</span><select data-evo-select="${currentKey}">${list.map(option).join("")}</select></label></div></section>`;
    };
  ...
  function renderScope() {
    ...
    else target.innerHTML = `<strong>Escopo por codificação</strong><span>Nenhuma LD está disponível no seletor principal nesta sessão. A evolução usa apenas as regras de codificação GRCON e sinaliza esse modo; carregue as LDs para validação documental direta.</span>`;
  }

  function renderKpis() {
    const target = document.getElementById("spw-evo-kpis"), net = document.getElementById("spw-evo-net"); if (!target || !net) return;
    const c = state.comparison || {}, s = c.sigem, p = c.pw, r = c.relation || {};
    target.innerHTML = `<button class="spw-evo-kpi sigem" data-evo-list="sigem-new"><span>Novos no SIGEM</span><strong>+${fmt(s?.added?.length || 0)}</strong><small>Ver registros</small></button><button class="spw-evo-kpi pw" data-evo-list="pw-new"><span>Novos no PW</span><strong>+${fmt(p?.added?.length || 0)}</strong><small>Ver registros</small></button><button class="spw-evo-kpi pending" data-evo-list="missing-pw"><span>Ainda não no PW</span><strong>${fmt(r.newSigemMissingPw?.length || 0)}</strong><small>Novos SIGEM sem correspondência atual no PW</small></button>`;
  ...
  function auditArticle(label, snapshot) {
    ...
    const reasons = Object.entries(a.discardReasons || {}).sort((x,y)=>y[1]-x[1]).slice(0,4).map(([reason,count])=>`${reason}: ${fmt(count)}`).join(" · ");
    return `<article><strong>${label} · ${esc(snapshot.fileName || "base")}</strong><p>${fmt(a.rawRecords)} brutos · ${fmt(a.acceptedRecords)} aceitos · ${fmt(a.uniqueDocuments)} documentos únicos · ${fmt(a.technicalDuplicates)} duplicidade(s) técnica(s) · ${fmt(a.discardedRecords)} descartado(s). Validação: ${a.validationMode === "ld" ? "LD + codificação" : "codificação"}.${reasons ? ` ${esc(reasons)}` : ""}</p></article>`;
  }
  function renderAudit() {
    const target = document.getElementById("spw-evo-audit"); if (!target) return;
    target.innerHTML = auditArticle("SIGEM atual", selected("sigem","current")) + auditArticle("PW atual", selected("pw","current"));
  }
  ...
  const LIST_LABELS = {
    ...
    "removed-sigem": ["Não encontrados nesta base SIGEM", "Ocorrências presentes na base anterior e ausentes na atual; não são tratadas automaticamente como exclusão definitiva."],
    "removed-pw": ["Não encontrados nesta base PW", "Ocorrências presentes na base anterior e ausentes na atual; não são tratadas automaticamente como exclusão definitiva."],
    discarded: ["Descartados do escopo", "Registros que não pertencem ao universo GRCON/LD ou falharam nas regras de escopo."],
  };
  ...
  function renderTabs() {
    ...
    const c = state.comparison || {}, r = c.relation || {};
    const counts = { "sigem-new": c.sigem?.added?.length || 0, "pw-new": c.pw?.added?.length || 0, both: r.newInBoth?.length || 0, "missing-pw": r.newSigemMissingPw?.length || 0, "removed-sigem": c.sigem?.removed?.length || 0, "removed-pw": c.pw?.removed?.length || 0, discarded: (selected("sigem","current")?.rejected?.length || 0) + (selected("pw","current")?.rejected?.length || 0) };
    target.innerHTML = Object.entries(LIST_LABELS).map(([key, [label]]) => `<button type="button" class="${state.listMode === key ? "active" : ""}" data-evo-list="${key}">${esc(label)} · ${fmt(counts[key])}</button>`).join("");
  }
  ...
  function renderTable() {
    ...
    const [title, subtitle] = LIST_LABELS[state.listMode] || ["Registros", ""];
    document.getElementById("spw-evo-list-title").textContent = title;
    document.getElementById("spw-evo-list-subtitle").textContent = subtitle;
    document.getElementById("spw-evo-list-count").textContent = `${fmt(rows.length)} registro(s) após filtro`;
    const pages = Math.max(1, Math.ceil(rows.length / PAGE_SIZE)); state.page = Math.min(Math.max(1,state.page), pages);
    const start = (state.page - 1) * PAGE_SIZE, pageRows = rows.slice(start, start + PAGE_SIZE);
    if (!pageRows.length) target.innerHTML = `<div class="spw-evo-empty"><strong>Nenhum registro nesta relação.</strong>Os números acima e esta lista usam a mesma origem; por isso uma contagem zero gera uma lista vazia.</div>`;
    else target.innerHTML = `<table class="spw-evo-table"><thead><tr><th>Código</th><th>Rev.</th><th>Tipo</th><th>Status</th><th>Disciplina</th><th>TAG</th><th>EAP</th><th>Data</th><th>Origem</th></tr></thead><tbody>${pageRows.map((row,index) => `<tr data-evo-row="${start+index}"><td><strong>${esc(row.document || "—")}</strong></td><td>${esc(row.revision || "—")}</td><td>${esc(row.documentType || row.documentClass || "—")}</td><td>${esc(row.status || row.reason || "—")}</td><td>${esc(row.discipline || "—")}</td><td>${esc(row.tag || "—")}</td><td>${esc(row.eap || "—")}</td><td>${esc(row.date || "—")}</td><td>${esc((row.system || "").toUpperCase() || "—")}</td></tr>`).join("")}</tbody></table>`;
  ...
  function openDetail(row) {
    ...
    const fields = [
      ["Código", row.document], ["Revisão", row.revision], ["Título", row.title], ["Tipo", row.documentType || row.documentClass], ["TAG", row.tag], ["EAP", row.eap], ["Disciplina", row.discipline], ["Status", row.status], ["Data", row.date], ["Origem", (row.system || "").toUpperCase()], ["Emissão PW", row.system === "pw" ? (row.emitted ? `Emitido (${row.lastEmission || "evidência"})` : row.lastEmission || "Não indicada") : "—"], ["LD", row.ldSource ? `${row.ldSource}${row.ldSheet ? ` · ${row.ldSheet}` : ""}${row.ldRow ? ` · linha ${row.ldRow}` : ""}` : (row.ldValidated ? "Validado" : "—")], ["Prazo LD", row.ldPrazo], ["Motivo de descarte", row.reason], ["Chave da ocorrência", row.occurrenceKey], ["Correspondência SIGEM × PW", row.matchedPw ? `${row.matchedPw.document} · Rev. ${row.matchedPw.revision}` : state.listMode === "missing-pw" ? "Ainda não identificada no PW atual" : "—"]
    ];
    body.innerHTML = `<div class="spw-evo-detail">${fields.map(([label,value]) => `<div><span>${esc(label)}</span><strong>${esc(value || "—")}</strong></div>`).join("")}</div>`;
    overlay.hidden = false;
  }

  async function exportVisible() {
    const rows = state.filteredRows.slice();
    if (!rows.length) { notify("Não há registros filtrados para exportar.", "warning"); return; }
    try {
      if (root.GRCONModuleLoader) await root.GRCONModuleLoader.ensure("xlsx");
      const data = rows.map((row) => ({
        Código: row.document || "", Revisão: row.revision || "", Classe: row.documentClass || "", Tipo: row.documentType || "", Título: row.title || "", Status: row.status || "", Disciplina: row.discipline || "", TAG: row.tag || "", EAP: row.eap || "", Data: row.date || "", Origem: (row.system || "").toUpperCase(), "Emissão PW": row.system === "pw" ? (row.emitted ? "Emitido" : row.lastEmission || "") : "", "LD origem": row.ldSource || "", "LD aba": row.ldSheet || "", "Prazo LD": row.ldPrazo || "", "Motivo descarte": row.reason || ""
      }));
      const ws = root.XLSX.utils.json_to_sheet(data); const wb = root.XLSX.utils.book_new(); root.XLSX.utils.book_append_sheet(wb, ws, "Evolução");
      const out = root.XLSX.write(wb, { bookType: "xlsx", type: "array" });
  ...
  function bind() {
    ...
    document.addEventListener("change", (event) => {
      const select = event.target.closest?.("[data-evo-select]");
      if (select) { state.selections[select.dataset.evoSelect] = select.value; recalculate(); return; }
      const map = { "spw-evo-filter-class":"documentClass", "spw-evo-filter-revision":"revision", "spw-evo-filter-status":"status", "spw-evo-filter-discipline":"discipline", "spw-evo-filter-tag":"tag", "spw-evo-filter-eap":"eap" };
      if (map[event.target.id]) { state.filters[map[event.target.id]] = event.target.value; state.page = 1; renderTable(); }
    });
    document.addEventListener("input", (event) => {
      const map = { "spw-evo-filter-query":"query", "spw-evo-filter-revision":"revision", "spw-evo-filter-status":"status", "spw-evo-filter-discipline":"discipline", "spw-evo-filter-tag":"tag", "spw-evo-filter-eap":"eap" };
      if (!map[event.target.id]) return; state.filters[map[event.target.id]] = event.target.value; state.page = 1; renderTable();
    });
    document.addEventListener("click", (event) => {
      const list = event.target.closest?.("[data-evo-list]");
      if (list) { state.listMode = list.dataset.evoList; state.page = 1; renderTabs(); renderTable(); document.getElementById("spw-evo-table")?.scrollIntoView({ behavior:"smooth", block:"nearest" }); return; }
      const page = event.target.closest?.("[data-evo-page]");
      if (page) { state.page += page.dataset.evoPage === "next" ? 1 : -1; renderTable(); return; }
      const row = event.target.closest?.("[data-evo-row]");
      if (row) { openDetail(state.filteredRows[Number(row.dataset.evoRow)]); return; }
    ...
    ["grcon:conference-updated","grcon:pw-base-updated"].forEach((name) => root.addEventListener(name, () => {
      clearTimeout(state.eventTimer); state.eventTimer = root.setTimeout(() => { void loadSnapshots(false); }, 700);
    }));
  }
