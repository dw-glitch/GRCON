(function (root) {
  "use strict";
  const STORAGE_KEY = "grcon.task.center.v1";
  const CHANNEL_NAME = "grcon-task-center-v1";
  ...
  function read() { try { const data = JSON.parse(root.localStorage.getItem(STORAGE_KEY) || "[]"); return Array.isArray(data) ? data : []; } catch (_) { console.debug("[TaskCenter] context:", _); return []; } }
  function persist() { try { root.localStorage.setItem(STORAGE_KEY, JSON.stringify(tasks.slice(0, MAX_HISTORY))); } catch (_) { console.debug("[TaskCenter] context:", _); /* o histórico visual nunca bloqueia a tarefa */ } }
  function broadcast() { if (channel) try { channel.postMessage({ runtime, tasks }); } catch (_) { console.debug("[TaskCenter] context:", _); /* sem sincronização */ } }
  ...
  try { channel = new BroadcastChannel(CHANNEL_NAME); channel.addEventListener("message", (event) => { if (!event.data || event.data.runtime === runtime || !Array.isArray(event.data.tasks)) return; const merged = new Map([...tasks, ...event.data.tasks].map((task) => [task.id, task])); tasks = [...merged.values()].slice(0, MAX_HISTORY); persist(); render(); }); } catch (_) { console.debug("[TaskCenter] context:", _); channel = null; }
