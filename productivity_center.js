(function (root) {
  ...
  function safeStorage(name) {
    try {
      ...
      console.debug("[ProductivityCenter] context:", _);
      return null;
    }
  }
  ...
  function appEntries(storage) {
    ...
    for (let index = 0; index < storage.length; index += 1) {
      ...
      try { result[key] = storage.getItem(key); } catch (_) { console.debug("[ProductivityCenter] context:", _); }
    }
    return result;
  }
  ...
  function byteSize(value) {
    try { return new Blob([String(value || "")]).size; } catch (_) { console.debug("[ProductivityCenter] byteSize fallback:", _); return String(value || "").length * 2; }
  }
  ...
  async function refreshStorageSummary() {
    ...
    let estimate = null;
    try { estimate = await root.navigator?.storage?.estimate?.(); } catch (_) { console.debug("[ProductivityCenter] context:", _); }
  ...
  function readJsonFile(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        catch (_) { console.debug("[ProductivityCenter] context:", _); reject(new Error("O arquivo selecionado não contém um JSON válido.")); }
      };
