(function (root) {
  ...
  async function recordCurrent(reason, changedSystem) {
    ...
    try {
      const result = await Core().recordActiveBases(app.state.sigem, app.state.pw, { recordedAt: new Date().toISOString(), reason, ldRecords: app.state.ld && app.state.ld.records || [] }); state.lastPerformance.recordMs = nowMs() - started; await loadHistory(); render();
  ...
  async function exportHistory() {
    ...
    try {
      ...
      const sourceRows = (items) => items.map((s) => ({ Data: s.importedAt, Arquivo: s.fileName, Fingerprint: s.fingerprint, "Registros válidos": s.metrics.validRecords, "Documentos comparáveis": s.metrics.comparableDocuments, ET: s.metrics.classes?.ET || 0, "N-1710": s.metrics.classes?.["N-1710"] || 0, Emitidos: s.metrics.emittedDocuments, "Versão cálculo": s.calculationVersion }));
