(function (root) {
  ...
  async function exportHistory() {
    ...
    try {
      ...
      const sourceRows = (items) => items.map((s) => ({ Data: s.importedAt, Arquivo: s.fileName, Fingerprint: s.fingerprint, "Registros brutos": s.metrics.rawRecords, "Documentos comparáveis": s.metrics.comparableDocuments, ET: s.metrics.classes?.ET || 0, "N-1710": s.metrics.classes?.["N-1710"] || 0, "Fora do escopo": s.metrics.outsideScope, Inválidos: s.metrics.invalidRecords, Emitidos: s.metrics.emittedDocuments, "Versão cálculo": s.calculationVersion }));
