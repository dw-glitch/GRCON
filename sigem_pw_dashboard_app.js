(function (root) {
  ...
  const state = {
    ...
    filters: { documentClass: "", query: "" }, activeList: "all", page: 1,
  };
  let shell = null;
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .spw-actions-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin-bottom:8px}.spw-kpi{position:relative;overflow:hidden;min-height:108px;padding:12px 13px;border:1px solid var(--border,#dce4eb);border-radius:13px;background:var(--surface,#fff);box-shadow:0 5px 18px rgba(32,56,85,.04)}.spw-kpi:before{content:"";position:absolute;left:0;top:0;bottom:0;width:4px;background:var(--danger-500,#c74b43)}.spw-kpi.warn:before{background:var(--spw-issued)}.spw-kpi.pw:before{background:var(--spw-pw)}.spw-kpi.pw.warn:before{background:var(--spw-issued)}.spw-kpi.ok:before{background:var(--spw-ok)}.spw-kpi span{display:block;font-size:.65rem;font-weight:900;letter-spacing:.04em;text-transform:uppercase;color:var(--text-muted,#687b8c)}.spw-kpi strong{display:block;margin-top:8px;font-size:1.5rem;line-height:1;color:var(--text-strong,#17324a)}.spw-kpi small{display:block;margin-top:8px;color:var(--text-muted,#66798a);font-size:.7rem;line-height:1.35}.spw-classification-note{margin:0 0 10px;padding:8px 11px;border:1px solid var(--border,#dce4eb);border-radius:10px;background:var(--surface-soft,#f6f9fb);color:var(--text-muted,#66798a);font-size:.71rem}.spw-classification-note strong{color:var(--text-strong,#183247)}
      ...
      @media(max-width:1350px){.spw-actions-grid{grid-template-columns:repeat(3,1fr)}}
  ...
  function createShell() {
    ...
    shell.innerHTML = `
      ...
      <section class="spw-system-grid" id="spw-system-grid" aria-label="Totais SIGEM e ProjectWise"></section><section class="spw-actions-grid" id="spw-actions-grid" aria-label="Situações exclusivas entre as bases"></section><p class="spw-classification-note" id="spw-classification-note"></p>
      <section class="spw-list-card"><header class="spw-list-head"><div><span class="spw-kicker">RELAÇÃO DETALHADA</span><strong id="spw-list-title">Todas as situações</strong><small>Cada código + revisão entra em uma única situação, sem sobreposição entre os cinco totais.</small></div></header><div class="spw-list-tabs" id="spw-list-tabs"></div><div class="spw-list-filters"><input id="spw-query" type="search" placeholder="Pesquisar código, revisão, status ou situação" aria-label="Pesquisar na lista"/><select id="spw-class" aria-label="Filtrar classe"><option value="">ET e N-1710</option><option value="ET">ET</option><option value="N-1710">N-1710</option></select><button class="text-button" id="spw-clear" type="button">Limpar filtros</button></div><div class="spw-table-wrap" id="spw-table"></div><div class="spw-pager"><span id="spw-page-info"></span><div class="spw-pager-actions"><button class="secondary-button" id="spw-prev" type="button">Anterior</button><button class="secondary-button" id="spw-next" type="button">Próxima</button></div></div></section>
  ...
  function situationClass(row) { if (row.situationKey === "bothEmitted") return "ok"; if (row.situationKey === "bothNotEmitted" || row.situationKey === "pwOnlyNotEmitted") return "warn"; return "danger"; }
  function renderList() {
