(function (root) {
  ...
  const state = {
    ...
    history: { version: 3, snapshots: [] }, model: null, result: null,
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:820px){.spw-system-grid{grid-template-columns:1fr}.spw-page-heading{display:grid}.spw-heading-actions{justify-content:flex-start}.spw-list-filters{grid-template-columns:1fr}.spw-history-row{grid-template-columns:70px minmax(0,1fr)}}
  ...
  async function importSigem(file) {
    ...
    try {
      ...
      state.sigem = await Core.saveSigemBase({ meta: result.parsed.meta, records: result.parsed.records }); state.history = await Core.loadHistory(); rebuildModel(); renderFromModel(true);
  ...
  async function importPw(file) {
    ...
    try { if (!/\.(?:csv|txt)$/i.test(file.name || "")) throw new Error("A relação ProjectWise deve ser fornecida em CSV."); const importedAt = new Date().toISOString(); const parsed = await parsePwFile(file, { fileName: file.name, fileSize: file.size, lastModified: file.lastModified, importedAt }); state.pw = await Core.savePwBase({ meta: parsed.meta, records: parsed.records }); state.history = await Core.loadHistory(); rebuildModel(); renderFromModel(true); root.dispatchEvent(new CustomEvent("grcon:pw-base-updated", { detail: { baseMeta: state.pw.meta, source: "sigem-pw-dashboard" } })); notify(`Base PW atualizada: ${fmt(state.model.pwEntries.size)} registros/revisões válidos no universo ET e N-1710.`, "success"); }
  ...
  function renderFromModel(resetPage) { if (!state.model) rebuildModel(); if (resetPage) state.page = 1; state.result = Core.aggregateModel(state.model, { documentClass: state.filters.documentClass }); renderBases(); renderSystems(); renderActions(); renderList(); }
