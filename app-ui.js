...
(() => {
    ...
    const escapeHtml = (value) => { const U = window.GrconUtils; return U && U.escapeHtml ? U.escapeHtml(value) : String(value ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;"); };
    ...
    function setTheme(theme, notify = false, persist = true) {
        ...
        if (persist) {
            try {
                ...
                console.debug("[AppUI] storage:", _); /* armazenamento opcional */
            }
        }
    ...
    function installRuntimeTools() {
        ...
        catch (_) { console.debug("[AppUI] storage:", _); /* armazenamento opcional */ }
        setTheme(saved);
