...
(function () {
  ...
  const ASSET = new URL("grcon-mascot.png?v=2", document.baseURI).href;
  const FALLBACK = new URL("grcon-logo-app.png", document.baseURI).href;
  ...
  function styles() {
    ...
    el.id = STYLE;
    el.textContent = `
      .grcon-brand-mascot{display:inline-flex;flex:0 0 auto;align-items:center;justify-content:center;width:clamp(2.5rem,3.8vw,3.2rem);height:clamp(2.5rem,3.8vw,3.2rem);margin-inline:-.18rem -.05rem;pointer-events:none;user-select:none;overflow:visible}
      .grcon-brand-mascot img{display:block;width:100%;height:100%;max-width:none;object-fit:contain;opacity:1;visibility:visible;filter:drop-shadow(0 3px 7px rgb(12 32 48 / 14%))}
  ...
  function install() {
    ...
    img.addEventListener("load", () => {
      ...
      wrap.removeAttribute("data-load-error");
    }, { once: true });
    img.addEventListener("error", () => {
      if (img.dataset.fallbackApplied === "true") {
        wrap.dataset.loadError = "true";
  ...
  window.GRCONMascot = Object.freeze({ version: "2.0.0", assetUrl: ASSET, installHeader: install });
