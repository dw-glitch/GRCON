(function (root, factory) {
  ...
  const FIELD_DEFS = [
    ...
    { key: "allocationStatus", label: "Confirmação de alocação", standard: ["CONFIRMACAO DE DOCUMENTOS PREVISTOS", "CONFIRMACAO DE ALOCACAO"], aliases: ["STATUS DE ALOCACAO", "SITUACAO DE ALOCACAO", "CONDICAO DA ALOCACAO", "ALOCADO", "DOCUMENTO ALOCADO", "ALOCACAO CONFIRMADA", "ACEITE DA ALOCACAO"] },
  ...
  function allocationStatusHeaderScore(value) {
    ...
    if (!header) return 0;
    const confirmation = header.includes("CONFIRMACAO");
    const document = header.includes("DOCUMENTO");
    const expected = header.includes("PREVISTO");
    const allocation = header.includes("ALOCACAO");
    if (confirmation && document && expected) return 1200;
    if (document && expected && /\bALOCAD/.test(header)) return 1160;
    if (confirmation && document) return 1120;
    if (/\b(NUMERO|NRO|CODIGO|IDENTIFICADOR|ETAPA|FASE|DATA|SOLICITACAO|ARQUIVO)\b/.test(header)) return 0;
    if (confirmation && allocation) return 1080;
    if (header === "ALOCACAO CONFIRMADA") return 1060;
    if (header.includes("ACEITE") && allocation) return 1000;
    if (/^(STATUS|SITUACAO|CONDICAO) (DE|DA) ALOCACAO$/.test(header)) return 980;
    if (/^(ALOCADO|DOCUMENTO ALOCADO|STATUS ALOCADO)$/.test(header)) return 940;
    return 0;
  }

  function allocationNumberHeaderScore(value) {
    const header = normalizedHeader(value);
    if (!header || allocationStatusHeaderScore(header)) return 0;
    if (/\b(ETAPA|FASE|STATUS|SITUACAO|CONDICAO|CONFIRMACAO|ACEITE|DATA)\b/.test(header)) return 0;
    if (/\b(NUMERO|NRO|CODIGO|IDENTIFICADOR)\b/.test(header) && header.includes("ALOCACAO")) return 1100;
    if (header === "ALOCACAO") return 1050;
    if (header.includes("ALOCACAO")) return 900;
    return 0;
  }
  ...
  function matchHeader(value) {
    ...
    const allocationStatusScore = allocationStatusHeaderScore(value);
    if (allocationStatusScore) return { field: "allocationStatus", kind: allocationStatusScore >= 1080 ? "standard" : "alias", score: allocationStatusScore };
    const allocationNumberScore = allocationNumberHeaderScore(value);
    if (allocationNumberScore) return { field: "allocation", kind: allocationNumberScore >= 1050 ? "standard" : "alias", score: allocationNumberScore };
  ...
  function mapHeaders(headers) {
    ...
    headers.forEach((header, column) => {
      ...
      if (!match) return;
      const current = matches[match.field];
      if (current && (current.score > match.score || (current.score === match.score && current.column > column))) return;
  ...
  function sheetColumnsInRows(sheet, startRow, endRow) {
    const columns = new Set();
    Object.keys(sheet || {}).forEach((address) => {
      if (!address || address[0] === "!") return;
      let decoded;
      try { decoded = root.XLSX.utils.decode_cell(address); } catch (_) { return; }
  ...
  function rowHeaders(sheet, row, columns) {
    const headers = [];
    (columns || []).forEach((column) => { headers[column] = cellValue(sheet, row, column); });
    return headers;
  }
  ...
  function rowScore(mapped) {
    ...
    if (columns.allocationStatus !== undefined) score += 10;
  ...
  function inspectSheet(workbook, sheetName) {
    ...
    const populatedColumns = sheetColumnsInRows(sheet, range.s.r, endRow);
    const candidates = [];
    for (let row = range.s.r; row <= endRow; row += 1) {
      const headers = rowHeaders(sheet, row, populatedColumns);
  ...
  async function readWorkbook(file) {
    if (!root.XLSX && root.GRCONModuleLoader) {
      await root.GRCONModuleLoader.ensure("xlsx");
    }
    if (!root.XLSX) throw new Error("Biblioteca XLSX não está disponível.");
  ...
  function dispatch(file) {
    ...
    document.dispatchEvent(new CustomEvent("grcon:ld-compatibility", { detail: { file, ready: ready(file), inspection: inspectionFor(file) } }));
  }
  ...
  function fieldOptions(sheet, selected) {
    ...
    return `<option value="">Não associar</option>${available.map((item) => `<option value="${item.column}" ${Number(selected) === item.column ? "selected" : ""}>${escapeHtml(item.header)} · coluna ${item.column + 1}</option>`).join("")}`;
  }
    ...
    body.innerHTML = `
      ...
      <div class="compatibility-sheets">${relevant.map((sheet) => `
        <section class="compatibility-sheet" data-compat-sheet="${escapeHtml(sheet.name)}">
          ...
          <div class="compatibility-fields">${FIELD_DEFS.map((field) => `
            <label><span>${escapeHtml(field.label)}</span><select data-compat-field="${field.key}">${fieldOptions(sheet, sheet.columns[field.key])}</select></label>
          `).join("")}</div>
        </section>
      `).join("")}</div>`;
  }
  ...
  function profileFromDrawer() {
    ...
    document.querySelectorAll("[data-compat-sheet]").forEach((card) => {
      ...
      card.querySelectorAll("[data-compat-field]").forEach((select) => {
        if (select.value !== "") columns[select.dataset.compatField] = Number(select.value);
      });
