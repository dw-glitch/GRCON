...
assert.match(ui, /top: 31%/);
assert.match(ui, /width: 24%/);
assert.match(ui, /height: 24%/);
assert.match(ui, /transform-origin: 50% 50%/);
assert.match(ui, /processingOrb\.className = "grcon-mascot-processing-orb"/);
assert.match(ui, /\(poseMotion \|\| motion \|\| mascot\)\.appendChild\(processingOrb\)/);
assert.match(ui, /animation: grcon-mascot-orb-spin 820ms linear infinite/);
const orbFrames = ui.match(/@keyframes grcon-mascot-orb-spin \{([\s\S]*?)\n      \}/)?.[1] || "";
