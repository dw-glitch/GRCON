...
const browser = read("scripts/validar-cloudflare-browser.cjs");
...
assert.match(validate, /validar-cloudflare-browser\.cjs/);
assert.match(validate, /wrangler@4\.34\.0 dev/);
assert.match(validate, /1440|GRCON_EXPECTED_COMMIT/);
assert.match(browser, /width: 1440, height: 900/);
assert.match(browser, /width: 390, height: 844/);
assert.match(browser, /navigator\.serviceWorker\.ready/);
assert.match(browser, /deployment-meta\.json/);
assert.match(browser, /grcon-mascot-idle-alpha\.webm/);
assert.match(browser, /MISSING_SESSION/);
